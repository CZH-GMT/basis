...
class MyAdapter extends RecyclerView.Adapter {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        if (viewType == 0) {
            ...
            View inflate = LayoutInflater.from(context).inflate(R.layout.rcyitem, parent, false);
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if (position == 0) {
            ...
            Bean.DataBean.CatalogBean bean = list.get(position-1);
            ViewHolder1 viewHolder1= (ViewHolder1) holder;
            viewHolder1.tv.setText(bean.getName());
            Glide.with(context).load(bean.getIcon()).into(viewHolder1.image);
        }
    }
